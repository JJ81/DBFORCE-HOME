...
$insertId=$media->setList($title, $thumbnail, $html, $author, $hits, $external_link, $link_origin, $date, $desc);

if(!empty($insertId)){
    Redirect(ROOT . 'admin/media/modify.php?id=' . $insertId);
}else{
    AlertMsgAndRedirectTo(ROOT . 'admin/media/', '알 수 없는 오류가 발생했습니다. 언론보도를 다시 작성해 주세요.');
}
