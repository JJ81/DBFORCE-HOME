...
if(count($check_result) == 0){
    ...
    // insert
    $insert_query="insert into `platform_settings` (`marketing`, `admin_id`, `company_id`) values (:privacy, :admin_id, :company_id)";
    $insert_value=array(
        ':privacy' => $html,
        ':admin_id' => $admin_id,
    ...
    $update_query="update `platform_settings` set `marketing`=:marketing where `id`=:setting_id and `company_id`=:company_id;";
    $value=array(
        ':marketing' => $html,
...
AlertMsgAndRedirectTo(ROOT . 'admin/settings/marketing.php', '마케팅이용방침이 정상적으로 등록되었습니다.');
exit;