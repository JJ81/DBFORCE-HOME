...
$insertId=$inc->setList($title, $thumbnail, $html, $author, $hits, $external_link, $link_origin, $date, $type, $id);

if(!empty($insertId)){
    Redirect(ROOT . 'admin/' . PAGE_NAME . '/modify_answer.php?id=' . $id);
}else{
    AlertMsgAndRedirectTo(ROOT . 'admin/' . PAGE_NAME, '알 수 없는 오류가 발생했습니다. 글을 다시 작성해 주세요.');
}
