...
// 두 개의 갯수가 맞지 않을 경우
if(count($ids) === 0){
    AlertMsgAndRedirectTo(ROOT . 'admin/vip_data/', '잘못된 접근입니다.');
    exit;
}
...
for($i=0,$size=count($ids);$i<$size;$i++){
    ...
    $result_update=$vip->modVipList($tmp_name, $tmp_tel, $tmp_id);
    error_log('$result_update');
    error_log($result_update);
}

AlertMsgAndRedirectTo(ROOT . 'admin/vip_data/', '정상적으로 처리되었습니다.');
