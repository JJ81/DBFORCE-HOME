...
if(count($notice_info) === 0){
    AlertMsgAndRedirectTo(ROOT . 'admin/notice/', '존재하지 않는 게시물입니다.');
    exit;
}
...
$thumbnail=null;
$UPLOAD_DIR_IMAGE=NOTICE_IMG_PATH; // 공지사항 이미지 업로드 경로
$newImgName=null;
...
if($newImgName !== null){
    if (move_uploaded_file($_FILES['thumbnail']['tmp_name'], $UPLOAD_DIR_IMAGE . $newImgName )) {
        ...
        AlertMsgAndRedirectTo(ROOT . 'admin/notice/', '잘못된 설정으로 이미지 업로드에 실패하였습니다.');
        exit;
    }
}else{
    if(!empty($delete_thumbnail)){
        ...
        // 새로운 이미지가 없는 경우
        $thumbnail=$notice_info[0]['thumbnail']; // 기존의 정보를 연결
    }
}
...
$result_update_notice=$notice->updateList($title, $thumbnail, $html, $author, $hits, $external_link, $link_origin, $id, $date);

error_log('$result_update_notice');
error_log($result_update_notice);

if(!empty($result_update_notice)){
    Redirect(ROOT . 'admin/notice/modify.php?id=' . $id);
}else{
    AlertMsgAndRedirectTo(ROOT . 'admin/notice/modify.php?id=' . $id, '변경사항이 없습니다.');
}
