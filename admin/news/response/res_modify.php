...
$thumbnail=null;
$UPLOAD_DIR_IMAGE=NEWS_IMG_PATH; // 공지사항 이미지 업로드 경로
$newImgName=null;
...
if($newImgName !== null){
    if (move_uploaded_file($_FILES['thumbnail']['tmp_name'], $UPLOAD_DIR_IMAGE . $newImgName )) {
        ...
        AlertMsgAndRedirectTo(ROOT . 'admin/news/', '잘못된 설정으로 이미지 업로드에 실패하였습니다.');
        exit;
    }
}else{
    if(!empty($delete_thumbnail)){
        ...
        // 새로운 이미지가 없는 경우
        $thumbnail=$news_info[0]['thumbnail']; // 기존의 정보를 연결
    }
}

error_log('[INFO] Check news data when updating it');
error_log($title);
...
$result_update_news=$news->updateList($title, $thumbnail, $html, $author, $hits, $external_link, $link_origin, $id, $date);

error_log('$result_update_news');
error_log($result_update_news);

if(!empty($result_update_news)){
    Redirect(ROOT . 'admin/news/view.php?id=' . $id);
}else{
    AlertMsgAndRedirectTo(ROOT . 'admin/news/view.php?id=' . $id, '변경사항이 없습니다.');
}
