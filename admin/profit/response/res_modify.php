...
$profit=new Profit();
$news_info=$profit->getListById($id);
...
$result_update_news=$profit->updateList($title, $thumbnail, $html, $author, $hits, $external_link, $link_origin, $id, $date);

error_log('$result_update_news');
error_log($result_update_news);

if(!empty($result_update_news)){
    Redirect(ROOT . 'admin/'.TARGET_FOLDER.'/view.php?id=' . $id);
}else{
    AlertMsgAndRedirectTo(ROOT . 'admin/'.TARGET_FOLDER.'/view.php?id=' . $id, '변경사항이 없습니다.');
}
