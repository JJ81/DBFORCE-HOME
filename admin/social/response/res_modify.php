...
$social=new Social();
$social_info=$social->getListById($id);

if(count($social_info) === 0){
...
if($newImgName !== null){
    ...
    if(!empty($delete_thumbnail)){
        ...
        // 새로운 이미지가 없는 경우
        $thumbnail=$social_info[0]['thumbnail']; // 기존의 정보를 연결
    }
}
...
$result_update_notice=$social->updateList($title, $thumbnail, $html, $author, $hits, $external_link, $link_origin, $id, $date);

error_log('$result_update_notice');
error_log($result_update_notice);

if(!empty($result_update_notice)){
    Redirect(ROOT . 'admin/social/modify.php?id=' . $id);
}else{
    AlertMsgAndRedirectTo(ROOT . 'admin/social/modify.php?id=' . $id, '변경사항이 없습니다.');
}
