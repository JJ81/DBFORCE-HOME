...
if(count($info) === 0){
    AlertMsgAndRedirectTo(ROOT . 'qna_profit.php', '존재하지 않는 게시물입니다.');
    exit;
}

// 삭제 권한 찾기
if($member_cid !== $info[0]['writer_id']){
    AlertMsgAndRedirectTo(ROOT . 'qna.php', '게시글을 삭제할 권한이 없습니다.');
    exit;
}
...
if(!empty($result)){
    AlertMsgAndRedirectTo(ROOT . 'qna.php', '선택된 게시물이 삭제되었습니다.');
}else{
