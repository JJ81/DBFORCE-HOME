...
if(empty($_POST['title']) or empty($_POST['position']) or empty($_POST['order']) ){
    echo json_encode(array(
        'success' => false,
        'code' => 403,
...
// 이미지 업로드 처리
    ...
    if (move_uploaded_file($_FILES["thumbnail"]["tmp_name"], $path . $newImgName)) {
        ...
        $result=$review->setReviewInfo($title, $position, $newImgName, $order, $company_id);
...
}else{ // 후기 이미지가 없을 때의 처리

    $result=$review->setReviewInfo($title, $position, NULL, $order, $company_id);
