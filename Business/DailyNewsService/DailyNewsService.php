...
namespace JCORP\Business\DailyNewsService;
use JCORP\Database\DBConnection as DBConn;

class DailyNewsService extends DBConn
{
    private $db=null;
    ...
    public function getList(int $offset, int $size){
        $query=
            ...
            "from `platform_daily_news`, (SELECT @rn:=0) r " .
    ...
    public function getTotalListCount(){
        $query="select count(*) as `total` from `platform_daily_news` where `is_delete`=0;";
    ...
    public function getListById(int $id){
        $query="select * from `platform_daily_news` where `id`=$id and `is_delete`=0;";
        return $this->db->query($query);
    }
    ...
    public function setList(string $title, string $thumbnail=null, string $contents, string $author=null, int $count=null, string $external_link=null, string $link_origin=null, string $date=null){
        $query="insert into `platform_daily_news` (`title`, `thumbnail`, `contents`, `author`, `count`, `external_link`, `link_origin`, `registered_dt`) ".
    ...
    public function updateList(string $title, string $thumbnail=null, string $contents, string $author=null, int $count=null, string $external_link=null, string $link_origin=null, int $id, string $date=null){
        $query="update `platform_daily_news` set `title`=:title, `thumbnail`=:thumbnail, `contents`=:contents, `author`=:author, `count`=:count, `external_link`=:external_link, `link_origin`=:link_origin, `registered_dt`=:date where `id`=:id;";
        $value=array(
            ':title' => $title,
            ':thumbnail' => $thumbnail,
            ':contents' => $contents,
    ...
    public function delList(int $id){
        $query="update `platform_daily_news` set `is_delete`=true where `id`=:id;";
        $value=array(
            ':id' => $id
        );
        return $this->db->update($query, $value);
    ...
    public function delThumbnailById($id){
        $query="update `platform_daily_news` set `thumbail`=null where `id`=:id;";
        $value=array(
            ':id' => $id
        );
        return $this->db->update($query, $value);
    ...
    public function increaseCount($id){
        $query="update `platform_daily_news` set `count`=`count`+1 where `id`=$id;";
    ...
    public function getBoardPrevAndNext(int $id){
        ...
        $query_prev=
            "select * from `platform_daily_news` where `id` in ( " .
            " select max(`id`) from `platform_daily_news` where `id` < $id and `external_link` is null and `is_delete`=0 " .
            ")";
        $query_next=
            "select * from `platform_daily_news` where `id` in ( " .
            " select min(`id`) from `platform_daily_news` where `id` > $id and `external_link` is null and `is_delete`=0 " .
            ")";
