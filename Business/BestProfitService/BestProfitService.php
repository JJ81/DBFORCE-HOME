...
namespace JCORP\Business\BestProfitService;
use JCORP\Database\DBConnection as DBConn;

class BestProfitService extends DBConn
{
    private $db=null;
    ...
    public function getList(int $offset, int $size){
        $query=
            ...
            "from `platform_best_profit`, (SELECT @rn:=0) r " .
    ...
    public function getTotalListCount(){
        $query="select count(*) as `total` from `platform_best_profit` where `is_delete`=0;";
    ...
    public function getListById(int $id){
        $query="select * from `platform_best_profit` where `id`=$id and `is_delete`=0;";
        return $this->db->query($query);
    }
    ...
    public function setList(string $title, string $thumbnail=null, string $contents, string $author=null, int $count=null, string $external_link=null, string $link_origin=null, string $date=null, string $stock_title=null, string $max_profit_rate=null, string $max_profit_price=null){
        $query="insert into `platform_best_profit` (`title`, `thumbnail`, `contents`, `author`, `count`, `external_link`, `link_origin`, `registered_dt`, `stock_title`, `max_profit_rate`, `max_profit_price`) ".
    ...
    public function updateList(string $title, string $thumbnail=null, string $contents, string $author=null, int $count=null, string $external_link=null, string $link_origin=null, int $id, string $date=null, string $stock_title=null, string $max_profit_rate=null, string $max_profit_price=null){
        $query="update `platform_best_profit` set `title`=:title, `thumbnail`=:thumbnail, `contents`=:contents, `author`=:author, `count`=:count, 
    ...
    public function delList(int $id){
        $query="update `platform_best_profit` set `is_delete`=true where `id`=:id;";
        $value=array(
            ':id' => $id
        );
        return $this->db->update($query, $value);
    ...
    public function delThumbnailById($id){
        $query="update `platform_best_profit` set `thumbail`=null where `id`=:id;";
        $value=array(
            ':id' => $id
        );
        return $this->db->update($query, $value);
    ...
    public function increaseCount($id){
        $query="update `platform_best_profit` set `count`=`count`+1 where `id`=$id;";
    ...
    public function getBoardPrevAndNext(int $id){
        ...
        $query_prev=
            "select * from `platform_best_profit` where `id` in ( " .
            " select max(`id`) from `platform_best_profit` where `id` < $id and `external_link` is null and `is_delete`=0 " .
            ")";
        $query_next=
            "select * from `platform_best_profit` where `id` in ( " .
            " select min(`id`) from `platform_best_profit` where `id` > $id and `external_link` is null and `is_delete`=0 " .
            ")";
     ...
     * @return array
     */
    public function getBestRank(int $rank){
        $query="select * from `platform_best_profit` where `is_delete`=0 order by cast(`max_profit_rate` as unsigned) desc limit 0, $rank;";
        return $this->db->query($query);
    }
