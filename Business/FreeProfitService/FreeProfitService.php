...
namespace JCORP\Business\FreeProfitService;
use JCORP\Database\DBConnection as DBConn;

class FreeProfitService extends DBConn
{
    private $db=null;
    ...
    public function getList(int $offset, int $size){
        $query=
            ...
            "from `platform_free_profit`, (SELECT @rn:=0) r " .
    ...
    public function getProbability(){
        $query="select 
                ...
                from `platform_free_profit` where `is_delete`=0;";
        return $this->db->query($query);
    }
    ...
    public function getTotalListCount(){
        $query="select count(*) as `total` from `platform_free_profit` where `is_delete`=0;";
    ...
    public function getListById(int $id){
        $query="select * from `platform_free_profit` where `id`=$id and `is_delete`=0;";
        return $this->db->query($query);
    }
    ...
    public function setList(string $title, string $thumbnail=null, string $contents, string $author=null, int $count=null, string $external_link=null, string $link_origin=null, string $date=null, string $stock_title=null, string $max_profit_rate=null, string $max_profit_price=null){
        $query="insert into `platform_free_profit` (`title`, `thumbnail`, `contents`, `author`, `count`, `external_link`, `link_origin`, `registered_dt`, `stock_title`, `max_profit_rate`, `max_profit_price`) ".
    ...
    public function setList2(string $stock_title, string $purchase_price, string $purchase_date, string $sell_price, string $sell_date, string $profit_rate, string $author){
        $query="insert into `platform_free_profit` (`stock_title`, `purchase_price`, `purchase_date`, `sell_price`, `sell_date`, `profit_rate`, `author`) ".
    ...
    public function updateList(string $title, string $thumbnail=null, string $contents, string $author=null, int $count=null, string $external_link=null, string $link_origin=null, int $id, string $date=null, string $stock_title=null, string $max_profit_rate=null, string $max_profit_price=null){
        $query="update `platform_free_profit` set `title`=:title, `thumbnail`=:thumbnail, `contents`=:contents, `author`=:author, `count`=:count, 
    ...
    public function updateList2(string $stock_title, string $purchase_price, string $purchase_date, string $sell_price, string $sell_date, string $profit_rate, string $author, int $id){
        $query="update `platform_free_profit`  set `stock_title`=:stock_title, `purchase_price`=:purchase_price, `purchase_date`=:purchase_date, 
    ...
    public function delList(int $id){
        $query="update `platform_free_profit` set `is_delete`=true where `id`=:id;";
        $value=array(
            ':id' => $id
        );
        return $this->db->update($query, $value);
    ...
    public function delThumbnailById($id){
        $query="update `platform_free_profit` set `thumbail`=null where `id`=:id;";
        $value=array(
            ':id' => $id
        );
        return $this->db->update($query, $value);
    ...
    public function increaseCount($id){
        $query="update `platform_free_profit` set `count`=`count`+1 where `id`=$id;";
    ...
    public function getBoardPrevAndNext(int $id){
        ...
        $query_prev=
            "select * from `platform_free_profit` where `id` in ( " .
            " select max(`id`) from `platform_free_profit` where `id` < $id and `external_link` is null and `is_delete`=0 " .
            ")";
        $query_next=
            "select * from `platform_free_profit` where `id` in ( " .
            " select min(`id`) from `platform_free_profit` where `id` > $id and `external_link` is null and `is_delete`=0 " .
            ")";
    ...
    public function getTotalStock(){
        $query="select count(*) as `total` from (
                    select * from `platform_free_profit`
                    where `is_delete`=0
    ...
    public function getTotalProfitRate(){
        $query="select sum(`profit_rate`) as `total` from `platform_free_profit`
                where `is_delete`=0;";
        return $this->db->query($query);
    }
